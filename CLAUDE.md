...
- **`release-please-config.json` is intentionally different between `main` and `develop`** — `develop` stays a perpetual beta channel (`prerelease: true`), `main` is the stable channel (`prerelease: false`). Never sync this file between the two branches. If a `main`↔`develop` merge ever conflicts on it, keep the target branch's own copy.
