 ...
 * rows whose payee names an account of the user's that nothing ever syncs
 * into.
 *
 ...
 * suggestion is only ever offered, never linked automatically.
 */
export function TransferSuggestionsBanner({
  ...
  const payeeSuggestionsQuery = useQuery<PayeeTransferSuggestion[]>({
    queryKey: ["transfer-payee-suggestions", startDate, endDate],
    queryFn: () =>
      api<PayeeTransferSuggestion[]>(
        `/api/transactions/transfer-payee-suggestions?start_date=${startDate}&end_date=${endDate}`,
      ),
  });

  function invalidate() {
    void qc.invalidateQueries({ queryKey: ["transfer-suggestions"] });
    void qc.invalidateQueries({ queryKey: ["transfer-payee-suggestions"] });
    onLinked();
  }
  ...
  const confirmPayeeMatch = useMutation({
    mutationFn: (suggestion: PayeeTransferSuggestion) =>
      api(`/api/transactions/${suggestion.transaction.id}/transfer-link`, {
        method: "POST",
        body: { to_account_id: suggestion.to_account_id },
      }),
    onSuccess: invalidate,
  });

  const suggestions = suggestionsQuery.data ?? [];
  const payeeSuggestions = payeeSuggestionsQuery.data ?? [];
  if (suggestions.length === 0 && payeeSuggestions.length === 0) return null;
  ...
  return (
    <div className="bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-200 dark:border-indigo-900/50 rounded-2xl p-5 space-y-4">
      ...
      {payeeSuggestions.length > 0 && (
        <div className="space-y-3">
          <div>
            <h2 className="font-semibold text-indigo-900 dark:text-indigo-200">
              Transfers to an account you don't sync
            </h2>
            <p className="text-sm text-indigo-800/80 dark:text-indigo-300/80">
              The payee names one of your accounts, and nothing is ever imported there to
              match against. Confirming creates the missing transaction there.
            </p>
          </div>
          ...
          <ul className="space-y-2">
            {payeeSuggestions.map((suggestion) => (
              ...
              >
                <div className="min-w-0 text-sm">
                  <div className="truncate text-stone-800 dark:text-stone-200">
                    {accountName(suggestion.transaction)} → {suggestion.to_account_name}
                  </div>
                  <div className="text-xs text-stone-500 dark:text-stone-400">
                    {suggestion.transaction.date}
                    {suggestion.transaction.payee && ` · ${suggestion.transaction.payee}`}
                  </div>
                </div>
                <div className="flex items-center gap-3 whitespace-nowrap">
                  <span className="tabular-nums text-sm text-stone-900 dark:text-stone-100">
                    {formatCents(Math.abs(suggestion.transaction.amount_cents))}
                  </span>
                  <button
                    type="button"
                    disabled={confirmPayeeMatch.isPending}
                    onClick={() => confirmPayeeMatch.mutate(suggestion)}
                    className="border border-indigo-300 dark:border-indigo-600 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-indigo-900/40 disabled:opacity-50 rounded-lg px-3 py-1 text-sm"
                  >
                    Confirm
                  </button>
          ...
          {confirmPayeeMatch.error && (
            <p className="text-sm text-red-600 dark:text-red-400">
              {confirmPayeeMatch.error instanceof Error
                ? confirmPayeeMatch.error.message
